...
def test_runtime_loop_matrix_open_event_sends_text_and_raw_snapshot(
    ...
    assert matrix_client.texts[0]["txn_id"].endswith(":text")
    assert matrix_client.texts[0]["body"] == "Parking spot open: left_spot at 2026-05-18T19:00:00+00:00"
    assert len(matrix_client.uploads) == 1
    assert matrix_client.uploads[0]["content_type"] == "image/jpeg"
    assert matrix_client.uploads[0]["data"] == snapshot_files[0].read_bytes()
    assert len(matrix_client.images) == 1
    assert matrix_client.images[0]["txn_id"].endswith(":image")
    assert matrix_client.images[0]["info"]["mimetype"] == "image/jpeg"
...
def test_runtime_loop_matrix_upload_failure_logs_safe_context_and_retains_copied_snapshot(
    ...
    records = json_records(output)
    snapshot_files = list((tmp_path / "snapshots").glob("occupancy-open-event-left-spot-*.jpg"))
    failed = next(record for record in records if record["event"] == "matrix-delivery-failed")
