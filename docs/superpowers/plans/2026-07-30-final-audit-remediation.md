...
| `parking_spot_monitor/owned_file_cleanup.py` | One capped deterministic scan coordinating exact quarantine and interrupted-disposal recovery. |
| `parking_spot_monitor/owned_file_disposal.py` | Exact random disposal grammar, inode binding, crash restoration/finish, and the cooperative owned-directory threat contract. |
...
| `parking_spot_monitor/matrix_snapshot_storage.py` | Rooted, no-follow storage, durable child-directory creation, exact-size evidence reads, and typed deletion outcomes. |
...
- Preserves: one reentrant process-local transaction per owned directory identity spans durable manifest publication, producer rename, and its immediate outcome. Recovery holds the same transaction across manifest read, existence/error classification, disposal recovery, and manifest reconciliation. Only `FileNotFoundError` means absent; transient stat errors preserve pending work. The one-service-process deployment contract avoids unsupported cross-process mutation, while unrelated directory identities remain concurrent.
- Preserves: post-rename identity observations are exception-aware. `FileNotFoundError`, a conclusive nonregular artifact, and a conclusive inode mismatch remain distinct from transient `open`/`fstat`/`stat` errors. A transient observation failure returns pending without restore or manifest removal; later indexed recovery resolves the original transition after the fault clears, independent of legacy scan position.
