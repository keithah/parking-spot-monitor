...
**Architecture:** Keep the existing single-service Docker Compose topology and file-backed durability. Add small service-scoped owners for the detector, decision memory, Matrix command fetch, and archive index; keep all operator mutations on the capture thread; use optional, backward-readable persisted fields only where retry timing or an upload derivative must survive restart. Decompose large modules behind compatibility facades rather than introducing a database, broker, generic event bus, or service container.
...
- Archive indexes are bounded, in-memory, and reconciled from source-file signatures. No durable secondary archive format is introduced.
...
| `parking_spot_monitor/vehicle_history_index.py` | Reconciled per-file session cache, mutation summary, and bounded recent-session queries. |
| `parking_spot_monitor/jpeg_artifacts.py` | Atomic canonical JPEG publication using hardlink, reflink, then copy fallback. |
...
- [ ] **Step 6: Remove confirmed dead imports, not compatibility exports**
...
Run: `python -m pytest tests/test_module_decomposition.py tests/test_dependency_lock_contract.py tests/test_dependency_lock_generation.py tests/test_dependency_lock_validation.py tests/test_runtime_stream_escalation.py -q`
...
- Preserves: permissive direct loader behavior for compatibility (`strict=False` returns empty for malformed data); missing file remains a valid empty registry in both modes.
...
Run: `python -m pytest tests/test_owner_vehicles.py tests/test_runtime_owner_vehicle_cache.py tests/test_matrix.py -q`

Expected: FAIL because strict errors, byte bounding, LKG behavior, and a required provider do not exist.
...
Expected: PASS, including malformed initial registry -> warning/no alert, valid -> malformed -> valid replacement recovery, and default command-service provider wiring.

- [ ] **Step 7: Commit owner-boundary hardening**

```bash
git add parking_spot_monitor/owner_vehicles.py parking_spot_monitor/runtime_owner_vehicle_cache.py parking_spot_monitor/runtime_vehicle_events.py parking_spot_monitor/matrix_command_runtime.py parking_spot_monitor/matrix_commands.py parking_spot_monitor/matrix_command_catalog.py parking_spot_monitor/__main__.py tests/test_owner_vehicles.py tests/test_runtime_owner_vehicle_cache.py tests/test_matrix.py tests/test_startup.py
...
Add tests that two compact summary calls at one revision scan records once, mutation invalidates the cache, a failed persistence does not advance revision/cache, and one retryable phase failure causes exactly one `_persist_records` call.
...
Expected: FAIL because retry timing is worker-local, summaries rescan, retry outcomes lack one transactional mutation, and `outbox.py` is 957 lines.
...
- [ ] **Step 6: Add transactional phase results and revision-cached summaries**

`apply_phase_result` accepts one phase plus exactly one of delivered result, retry schedule, or terminal reason; it computes the record/phase state and persists once. Increment `_revision` and rebuild the ID index only after successful persistence. Cache compact summaries as `(revision, immutable_summary)` and return a fresh JSON-safe mapping so callers cannot mutate the cache.

- [ ] **Step 7: Prove durability and migration safety**

Run: `python -m pytest tests/test_outbox_persistence.py tests/test_matrix_outbox_delivery.py tests/test_module_decomposition.py -q`

Expected: PASS for legacy fixtures, retry restart/cap/jitter, phase idempotency, upload-before-image durability, corrupt quarantine, atomic failure reconciliation, cached summaries, and one persistence per phase outcome.
...
- Produces: `DecisionMemoryStore(path, *, checkpoint_interval_seconds, max_records=MAX_RECORDS, monotonic, logger)` with `append(record: DecisionMemoryRecord) -> None`, `extend(records: Sequence[DecisionMemoryRecord]) -> None`, `checkpoint_if_due() -> bool`, `flush() -> None`, `close() -> None`, and `records: tuple[DecisionMemoryRecord, ...]`.
...
- [ ] **Step 1: Add failing checkpoint and correction-cache tests**

```python
def test_decision_store_batches_frames_until_checkpoint(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    writes = 0
    monkeypatch.setattr(operator_decision_memory, "_write_memory", counted_writer(lambda: writes))
    store = DecisionMemoryStore(tmp_path / "operator-decision-memory.json", checkpoint_interval_seconds=300, monotonic=lambda: 0)
    store.extend((record("first"), record("second")))
    store.extend((record("third"),))
    assert writes == 0
    store.flush()
    assert writes == 1
...
Add store tests for external replacement reconciliation before flush, failed write retaining dirty state, corrupt startup quarantine, max-record truncation, due checkpoint, and close flush.
...
Expected: FAIL because each append reloads/rewrites the entire document and command dedupe scans all corrections.

- [ ] **Step 3: Implement one loaded bounded store**

Load and sanitize once in `DecisionMemoryStore.__init__`, keep a `deque(maxlen=max_records)` under the existing memory lock, and mark dirty on append. Before a flush, compare `(mtime_ns, size)` with the last successful signature; if externally replaced, load that file and merge its tail before local dirty records using stable record JSON as the dedupe key. On write failure keep `_dirty=True`; update the signature only after atomic success.
...
- [ ] **Step 4: Route runtime writes through the service owner**

Construct one store in `_main`, pass it through capture, state update, Matrix dispatch, and lifecycle calls, call `checkpoint_if_due()` once per successful or failed iteration, and close it through Task 4 cleanup isolation. Replace path-based runtime appends with `store.extend(frame_records)`; keep path functions for CLI/tests and external compatibility.
...
Expected: PASS, with zero per-frame decision writes before due checkpoint, one close flush, schema-v1 reload, external replacement reconciliation, and one correction replay load for repeated event checks.

- [ ] **Step 7: Commit service-scoped memory**

```bash
git add parking_spot_monitor/decision_memory_store.py parking_spot_monitor/operator_decision_memory.py parking_spot_monitor/runtime_decision_memory.py parking_spot_monitor/matrix_dispatch.py parking_spot_monitor/runtime_state_update.py parking_spot_monitor/runtime_lifecycle.py parking_spot_monitor/capture_loop.py parking_spot_monitor/__main__.py parking_spot_monitor/vehicle_history_models.py parking_spot_monitor/vehicle_history_correction_cache.py parking_spot_monitor/vehicle_history_corrections.py parking_spot_monitor/matrix_command_runtime.py parking_spot_monitor/config.py config.yaml.example tests/test_operator_decision_memory.py tests/test_vehicle_history.py tests/test_matrix.py tests/test_startup.py
git commit -m "perf: checkpoint service decision memory"
```

### Task 8: Reconcile an In-Memory Archive Summary and Recent-Session Index

**Files:**
- Create: `parking_spot_monitor/vehicle_history_index.py`
- Modify: `parking_spot_monitor/vehicle_history_storage.py`
- Modify: `parking_spot_monitor/vehicle_history_sessions.py`
- Modify: `parking_spot_monitor/vehicle_history_corrections.py`
- Modify: `parking_spot_monitor/vehicle_history_profiles.py`
- Modify: `parking_spot_monitor/vehicle_history_maintenance.py`
- Modify: `parking_spot_monitor/vehicle_history_storage_utils.py`
- Modify: `tests/test_vehicle_history.py`
- Modify: `tests/test_runtime_health_cache.py`

**Interfaces:**
- Produces: immutable `ArchiveMutationSummary(revision, active_count, closed_count, missing_image_reference_count, unknown_profile_count, oldest_started_at, newest_updated_at)`.
- Produces: `ArchiveSessionIndex.reconcile(active_dir: Path, closed_dir: Path, load_record: Callable[[Path], SessionRecord | None]) -> None`, `record_written(path: Path, record: SessionRecord, state: Literal["active", "closed"]) -> None`, `record_removed(path: Path) -> None`, `summary(revision: int) -> ArchiveMutationSummary`, `recent(limit: int) -> tuple[SessionRecord, ...]`, and `latest_for_spot(spot_id: str) -> SessionRecord | None`.
- Preserves: no durable index; `list_closed_sessions()` sorted-path behavior; public `estimate_for_profile` signature; archive JSON schemas.
- Improves: `profile_summary` passes its already-loaded effective closed records to private `_estimate_for_profile_records(...)` rather than scanning twice.

- [ ] **Step 1: Add failing reconciliation and single-scan tests**

```python
def test_archive_index_reconciles_external_add_replace_and_remove(tmp_path: Path) -> None:
    archive = VehicleHistoryArchive(tmp_path)
    first = create_closed_session(archive, spot_id="left", observed_at="2026-07-30T10:00:00Z")
    assert archive.recent_sessions(1) == (first,)
    external = write_external_closed_record(tmp_path, spot_id="right", observed_at="2026-07-30T11:00:00Z")
    assert archive.recent_sessions(1) == (external,)
    external_path(tmp_path, first).unlink()
    assert first not in archive.recent_sessions(10)
...
Expected: FAIL because no reconciled index/recent API exists and profile summary reloads closed records through `estimate_for_profile`.

- [ ] **Step 3: Implement a bounded per-file cache, not a persisted index**

Store `{Path: (mtime_ns, size, SessionRecord)}` for active and closed records. `reconcile` compares the current sorted `(name, mtime_ns, size)` signature, removes missing paths, parses only new/changed paths through `_load_record`, and rebuilds a bounded recent ordering by `(updated_at, session_id)`. Invalid records continue through existing quarantine behavior and never enter the index.

Update the index only after successful atomic record publication/unlink. If mutation bookkeeping raises, discard the index and force full reconciliation on next read; source files remain authoritative.

- [ ] **Step 4: Serve summary and recent consumers from reconciled records**

Expose `mutation_summary()` and `recent_sessions(limit)` on `VehicleHistoryArchive`. Reject non-positive or overlarge limits; use `MAX_RECENT_SESSIONS = 200`. Feed session counts/missing refs/profile counts into `health_snapshot`, retaining direct directory byte counts and maintenance metadata from their canonical sources.

Use `latest_for_spot` in `resolve_wrong_match_subject` only after exact session-ID lookup. Preserve timestamp and session-ID tie breaking.

- [ ] **Step 5: Reuse loaded effective records for estimates**

Extract:

```python
def _estimate_for_profile_records(self, profile_id: str | None, records: Sequence[SessionRecord], *, min_samples: int, min_profile_confidence: float) -> VehicleHistoryEstimate:
    return estimate_vehicle_history(records, profile_id=profile_id, min_samples=min_samples, min_profile_confidence=min_profile_confidence)
```

Public `estimate_for_profile` loads and delegates. `profile_summary` delegates with its existing `closed` list.

- [ ] **Step 6: Verify reconciliation and commit**

Run: `python -m pytest tests/test_vehicle_history.py tests/test_runtime_health_cache.py -q`

Expected: PASS for internal mutation, external add/replace/remove, corrupt quarantine, failed write invalidation, sorted compatibility, recent bounds, unchanged health fields, and one profile-summary scan.

```bash
git add parking_spot_monitor/vehicle_history_index.py parking_spot_monitor/vehicle_history_storage.py parking_spot_monitor/vehicle_history_sessions.py parking_spot_monitor/vehicle_history_corrections.py parking_spot_monitor/vehicle_history_profiles.py parking_spot_monitor/vehicle_history_maintenance.py parking_spot_monitor/vehicle_history_storage_utils.py tests/test_vehicle_history.py tests/test_runtime_health_cache.py
git commit -m "perf: reconcile archive session summaries"
```
...
Run: `python -m pytest tests/test_vehicle_history.py tests/test_matrix_outbox_delivery.py tests/test_image_budget.py -q`
...
Validate JPEG format/dimensions before publication. Create a temporary destination in the target directory, try `os.link`; on cross-device/unsupported errors try Linux `FICLONE` via `fcntl.ioctl`; otherwise stream `shutil.copyfileobj` in 1 MiB chunks. `fsync` the new file, chmod `0644`, atomically replace destination, and fsync its directory. Never modify the source inode.
...
- [ ] **Step 4: Persist the exact bytes selected for Matrix upload**
...
- [ ] **Step 5: Verify unchanged image semantics and commit**
...
```bash
git add parking_spot_monitor/jpeg_artifacts.py parking_spot_monitor/vehicle_history_images.py parking_spot_monitor/vehicle_history_sessions.py parking_spot_monitor/matrix_snapshots.py src/parking_monitor/matrix_outbox_delivery.py parking_spot_monitor/vehicle_history_maintenance_utils.py tests/test_vehicle_history.py tests/test_matrix_outbox_delivery.py tests/test_image_budget.py
...
The report table must contain the eleven task groups, exact commit SHA, focused tests, measured outcome, schema/compatibility result, and either `fixed`, `benchmark-only`, or `intentionally unchanged` with rationale. Confirm detector backend remains `.pt` unless Task 10 parity/resource gates passed and a separate approved production-switch change exists.

- [ ] **Step 6: Update operator documentation**

Document new timing keys/defaults, durable lifecycle behavior, `stop_grace_period`, command-fetch lag expectations, retry due persistence, decision checkpoints, archive reconciliation, canonical JPEG/derivative retention, `YOLO_CONFIG_DIR`, benchmark gate, serial verification, backup, upgrade, observation, and rollback. Update log guidance from per-frame INFO expectations to periodic aggregate summaries.
...
- [ ] The archive index is in-memory/reconciled, not a new durable recovery path.
