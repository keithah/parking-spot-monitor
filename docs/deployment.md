...
A confirmed occupied transition creates its base alert independently of history enrichment or profile matching. History or occupied-image enrichment failures degrade that optional context without cancelling the confirmed base alert. Both retained-snapshot preparation and outbox enqueue happen without Matrix network I/O. A recognized snapshot-preparation failure durably persists a text-only outbox record under the same event ID through the fallback path; an unrecognized preparation error or outbox persistence failure is still surfaced as a delivery error. Later, the delivery worker drains text before upload and image for a full snapshot record, while a degraded text-only record has no media phases. Treat preparation, persistence, or worker warnings and degraded health as work to repair; do not infer that text or image delivery succeeded merely because the base alert was created.
