...
A completed benchmark exits zero even when no alternative is eligible. Missing models or frames, a malformed manifest, a failed backend worker, a worker timeout, or malformed/non-finite evidence exits two. Preflight reads the manifest and every ordered frame from bounded `O_NOFOLLOW|O_NONBLOCK` descriptors, captures device/inode/size/mtime/ctime and SHA-256 identities, and creates one private read-only snapshot from those exact bytes before any worker starts. Every backend receives the same snapshot paths. FIFOs, sockets, devices, directories, symlinks, empty files, and oversized files are rejected as inputs without waiting for a writer. The report records the manifest hash, ordered frame hashes, corpus digest, frame count, corpus size, and calculated workload. The harness rechecks the original manifest, every original frame, and every model after every worker and immediately before report publication. Any mutation exits two and publishes no report.

Model preflight accepts only non-symlink regular files with the documented `.pt`, `.onnx`, and `.torchscript` suffixes; all three must have distinct resolved paths, inodes, and content. It streams each original from one bounded `O_NOFOLLOW|O_NONBLOCK` descriptor into a suffix-preserving private read-only snapshot, then verifies snapshot size and SHA-256 before starting workers. Workers receive only those snapshots. Evidence records the original model's bounded size and SHA-256 plus the equal snapshot SHA-256. Original model revalidation includes ctime, so a same-size mutate-and-restore attempt still invalidates the run; model snapshot directories are removed on every exit path.
