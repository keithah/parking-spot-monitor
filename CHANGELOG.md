...
- Deployed the final liveness/durability-closure detector image `sha256:b941c94a7a45e237bc02b21482b6068d3e1169d824467347bd748152ec24430c` with the existing operator bind mounts. Compose recreation took `1.26s`; fresh post-start frame evidence passed, the service stopped gracefully in `1.45s`, and it restarted healthy with zero restarts and no OOM. Five short samples ranged `354.7–391.4 MiB`, while a post-restart point was `350.5 MiB`; unequal windows and workloads make these health observations, not a peak-memory or resource-improvement claim.
- Added a checked-in transactional deployment helper for exact-image quiesced backup, exact-revision upgrade, rollback, and complete-data restore. It centralizes the post-start freshness gate, supports external bind sources and root-owned data, makes the selected environment file authoritative, transactionally handles different active and bundled model directories, fsyncs archives and both sides of bundle publication, requires fresh post-backup health, preserves primary/secondary failure context, uses operator-approved model provenance, and has injected failure-path tests.
- Closed the final adversarial persistence and cache findings: selected cleanup waits for bounded recovery scans; malformed outbox repair is conditional under a stable writer lock; registry parse/read failures reuse a bounded degraded snapshot; and decision-memory rollback uses a finite exchange budget with bounded, reader-visible conflict recovery so sustained external churn cannot block capture or hide displaced records.
