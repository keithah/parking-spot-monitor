...
- Deployed the final failure-window/cooperative-writer closure image `sha256:2f380efcdd1bce53d98a5b3f7f2f20f14806ca5f3e140c9026dc7142662471a5` with the existing operator bind mounts. Compose recreation took `1.25s`; fresh post-start frame evidence passed, the service stopped gracefully in `1.51s`, and it restarted healthy with zero restarts and no OOM. Five short samples ranged `350.5–372.0 MiB`, while a post-restart point was `350.3 MiB`; unequal windows and workloads make these health observations, not a peak-memory or resource-improvement claim.
- Added a checked-in transactional deployment helper for exact-image quiesced backup, exact-revision upgrade, rollback, and complete-data restore. It centralizes the post-start freshness gate, supports external bind sources and root-owned data, makes the selected environment file authoritative, transactionally handles different active and bundled model directories, requires a pre-existing symlink-free backup parent without group/other write permissions, fsyncs archives and both sides of bundle publication, requires fresh post-backup health, preserves primary/secondary failure context, uses operator-approved model provenance, and has injected failure-path tests.
- Closed the final adversarial persistence and cache findings: selected cleanup waits for bounded recovery scans; malformed outbox repair is conditional under a stable writer lock; registry parse/read failures reuse a bounded degraded snapshot; decision-memory publication rescues every exceptional post-exchange exit and compacts reader-visible conflicts without temporal loss; and occurrence-aware outbox reconciliation preserves disjoint or duplicate cooperative writes while rejecting same-occurrence divergence.
