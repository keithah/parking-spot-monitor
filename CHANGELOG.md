...
- Deployed the final reconciliation/operations closure image `sha256:a249a5ad7f0fedef234fda7e7f951b693377a8834d066876c8d0b8150fe6a17f` with the existing operator bind mounts. Compose recreation took `1.59s`; fresh post-start frame evidence passed, the service stopped gracefully in `1.92s`, and it restarted healthy with zero restarts and no OOM. Five short samples ranged `344.8–376.8 MiB`, while a post-restart point was `358.0 MiB`; unequal windows and workloads make these health observations, not a peak-memory or resource-improvement claim.
- Added a checked-in transactional deployment helper for exact-image quiesced backup, exact-revision upgrade, rollback, and complete-data restore. It centralizes the post-start freshness gate, supports external bind sources and root-owned data, makes the selected environment file authoritative, transactionally handles different active and bundled model directories, anchors backup staging/publication to a held identity-checked protected-parent descriptor, cleans failed rollback tags only after healthy recovery, fsyncs archives and both sides of bundle publication, preserves primary/secondary failure context, uses operator-approved model provenance, and has injected failure-path tests.
- Closed the final adversarial persistence and cache findings: selected cleanup waits for bounded recovery scans; malformed outbox repair is conditional under a stable writer lock; registry parse/read failures reuse a bounded degraded snapshot; decision-memory publication rescues exceptional exits, byte-bounds canonical and conflict output, serializes each candidate once, and retries incomplete conflict cleanup; and occurrence-aware outbox reconciliation preserves disjoint or duplicate cooperative writes while rejecting same-occurrence divergence.
