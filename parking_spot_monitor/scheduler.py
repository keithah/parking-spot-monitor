...
def quiet_window_notice_events(
    *, previous_active_window_ids: Iterable[str], current: QuietWindowStatus
) -> list[QuietWindowNoticeEvent]:
    """Return deterministic start/end notices by comparing persisted markers."""
...
def _active_window_id(window: QuietWindowConfig, now: datetime) -> str | None:
    ...
    if local_now.weekday() not in {WEEKDAY_INDEX[weekday] for weekday in window.weekdays}:
        return None
    if _ordinal_in_month(local_now) not in set(window.ordinals):
        return None
