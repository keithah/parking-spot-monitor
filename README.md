...
Matrix delivery has outbox-backed and direct paths. Runtime-generated open-spot and occupied-spot alerts are persisted to the local Matrix outbox before Matrix network I/O and are drained by the delivery worker. A confirmed occupied base alert is assembled independently of vehicle-history and profile enrichment. Retained-snapshot preparation and outbox enqueue perform no Matrix network I/O; a recognized occupied snapshot-preparation failure is durably persisted as a text-only outbox record under the same event ID. When a full snapshot record drains, the delivery worker sends text before the upload and image phases. Explicit live-proof operations remain a direct Matrix path. The outbox contract is documented in [`docs/outbox.md`](docs/outbox.md).
...
The copied `config.yaml` is ignored by Git and operator-owned. Runtime code does not rewrite it. Existing operator configs may omit the adaptive resource keys and receive validated model defaults until the operator chooses explicit overrides. An omitted `stable_frame_interval_seconds` resolves to the greater of its 60-second default and `frame_interval_seconds`, so a legacy active cadence above 60 seconds remains valid; an explicitly configured stable interval below the active interval is rejected. `config.yaml.example` is the tracked reference for explicit defaults. `config.yaml` should contain environment variable names only. The service resolves these required variables at startup; provide them through your shell, service manager, or ignored `.env` file used by the live-proof wrapper:
...
With the Docker default `--data-dir /data`, those relative values resolve to `/data/snapshots` and `/data/health.json`; with local `--data-dir ./data`, they resolve to `./data/snapshots` and `./data/health.json`. `storage.snapshot_retention_count` bounds Matrix event-specific JPEG snapshots in the snapshots directory (default `50`) while leaving `latest.jpg`, `debug_latest.jpg`, `state.json`, `health.json`, malformed filenames, and unrelated operator files untouched. Capture failures continue to use `stream.reconnect_seconds` backoff.

The active or uncertain cadence is `runtime.frame_interval_seconds` (default `30` seconds). Unknown or degraded state, an occupancy transition, a partial confirmation/release streak, or weak presence keeps this active cadence and resets settling. After `runtime.stable_settle_frames` consecutive successful stable frames (default `3`), adaptive polling uses `runtime.stable_frame_interval_seconds` (default `60` seconds, or the active interval when the key is omitted and active cadence is slower than 60 seconds). To roll back to fixed cadence, set `adaptive_polling_enabled: false`; the stable interval may remain omitted for this rollback. Setting `stable_frame_interval_seconds` equal to `frame_interval_seconds` is a second fixed-cadence rollback. Timing and cadence values must be finite. These controls change capture frequency, not the configured detection confidence, overlap, confirmation, release, or presence-suppression thresholds.

Every runtime iteration begins with the primary stream. Its atomic published image remains `/data/latest.jpg` (or `./data/latest.jpg` locally), and primary frames own routine timeline retention and debug overlays even when detection escalates. High-resolution verification publishes separately as `latest-high_resolution.jpg` and may become the authoritative frame for that iteration's detection, state transition, and event snapshot. Escalation is transition-driven for weak evidence that could change occupancy and is also periodic after state has settled, using `stream.escalation_verification_seconds` (default `600` seconds). A successful transition-driven verification resets that periodic deadline.

`runtime.debug_overlay_interval_seconds` controls periodic primary-frame overlays (default `60` seconds), while a state transition forces an overlay. Set `debug_overlay_interval_seconds: 0` to disable periodic debug overlays; transition-driven overlays remain enabled. Set `escalation_verification_seconds: 0` to disable periodic high-resolution verification; transition-driven escalation remains enabled when `stream.escalation_profile` is configured.

Runtime health now retains compact Matrix outbox counts and timestamps rather than record-level `items`. After deployment, `data/health.json` is expected to change from potentially hundreds of item details to compact counts such as `total` and `counts_by_state`; confirm the actual file size on the live host instead of treating this as a measured savings claim. Detailed durable outbox records remain in the outbox artifact and operator surfaces that explicitly request them. Vehicle-history health snapshots are also cached between archive mutations, and per-frame operator decision-memory records are persisted in one locked batch without changing their schema, sanitization, or bounds.
...
- `runtime`: `health_file`, `log_level`, and `startup_timeout_seconds` control operator health output, structured logging level, and startup guardrails. `frame_interval_seconds` is the active/uncertain cadence; `adaptive_polling_enabled`, `stable_frame_interval_seconds`, and `stable_settle_frames` control stable-state pacing and fixed-cadence rollback; `debug_overlay_interval_seconds` controls periodic routine overlays.

### 4. Interpret local artifacts

- `data/latest.jpg` is raw full-frame evidence from the most recent successful primary capture. Named profiles publish separately, for example `data/latest-high_resolution.jpg`, so a verification capture does not replace primary `latest.jpg` ownership. Published files are replaced atomically only after validation, preserving the last known-good JPEG on capture failure.
- `data/debug_latest.jpg` is a local RGB JPEG polygon overlay for tuning only. Periodic and transition-forced routine overlays use the primary image even if a high-resolution frame becomes authoritative for detection. Use it to confirm configured street polygons line up with the raw frame. Do not treat the overlay as Matrix alert evidence and do not publish it without review.
- `data/timeline/frames/` keeps a rolling missed-alert review buffer: one raw full-frame JPEG per UTC minute at primary resolution for the last 12 hours. High-resolution verification frames do not replace this routine timeline source. This is not a continuous video archive; it is a bounded local evidence buffer for cases where a spot opened but no Matrix alert was sent. At current 720p JPEG sizes, budget about 150–250 MiB for the 12-hour buffer.
...
- `data/health.json` summarizes current runtime status, selected decode mode, timestamps, failure counters, Matrix delivery errors, retention failures, state-save errors, last sanitized error, cached vehicle-history health, and compact Matrix outbox counts. It deliberately omits record-level outbox `items`. Inspect it with `python -m json.tool data/health.json`.
...
- `data/operator-decision-memory.json` is the bounded local source for `!parking why <spot_id>`, `!parking explain <spot_id>`, and `!parking recent`. Treat it as sanitized explanatory decision memory, not raw camera evidence, a complete audit log, or a place to store secrets; corrupt or oversized versions can be quarantined and reported as unavailable.
...
### 8. Run the secret-free M006 closeout smoke

Use the container-local M006 incident-intelligence closeout smoke when you need repeatable final-assembly evidence without live Matrix secrets, camera access, cloud detector work, or open-alert side effects:

```sh
python scripts/verify_m006_incident_intelligence_closeout.py
```

The smoke injects placeholder `RTSP_URL` and `MATRIX_ACCESS_TOKEN` values only for configuration validation, runs package/Docker validate-config checks, performs Docker/package contract checks, and may inspect optional local `/data`-style artifacts under `data/` such as `health.json`, `state.json`, `operator-feedback-labels.json`, and `data/timeline/frames/` when they already exist. It does not exercise live RTSP capture, live Matrix sync/delivery, media upload, live detector inference, detection-lab jobs, cloud services, or Matrix open-spot alert delivery.

Expected smoke diagnostics are bounded `M006_CLOSEOUT_START`, `M006_CLOSEOUT_PASS`, `M006_CLOSEOUT_FAIL`, `M006_CLOSEOUT_DATA`, and `M006_CLOSEOUT_RESULT` marker lines. Missing retained timeline frames are safe-empty closeout evidence, reported as `M006_CLOSEOUT_DATA timeline_frames status=safe-empty` rather than a failure. Missing or malformed optional retained artifacts are summarized by phase/status and should not leak RTSP URL values, Matrix token values, Authorization headers, raw Matrix response bodies, tracebacks, raw image bytes, or unbounded stdout/stderr.

### 9. Run the secret-free M007 Matrix outbox closeout smoke

Use the container-local M007 Matrix outbox closeout smoke when you need repeatable evidence that transient Matrix upload failure is persisted, surfaced in health, recovered by a fresh container, permanent failures are dead-lettered, corrupt outbox artifacts are quarantined, and pending/retrying retained image evidence survives pruning without live Matrix credentials or camera access:

```sh
python scripts/verify_m007_matrix_outbox_closeout.py
```

The smoke injects placeholder `RTSP_URL` and `MATRIX_ACCESS_TOKEN` values for config validation, builds the local Docker image `parking-spot-monitor:m007-outbox-smoke`, runs Docker Compose config checks, and then runs short `docker run` phases against a test-only mounted `/data` directory. The transient-failure phase simulates a Matrix upload timeout and expects `matrix-outbox.json` to report `retrying`, `matrix_upload_timeout`, and `text=delivered,upload=pending,image=pending`; it also writes `health.json` and checks the `matrix_outbox` payload. The recovery phase starts a fresh container against the same mounted `/data`, drains the durable outbox, skips the already-delivered text phase, calls only upload and image delivery, and expects the record to become `delivered`. Additional S05 phases prove a permanent upload failure becomes `dead_lettered` with `dead_letter_reason=matrix_upload_http_403` and `later_attempted=0`, corrupt persisted JSON is quarantined with `reason=invalid_json` and `quarantined_count=1`, and delayed upload uses retained original event evidence with `retained_original_upload=true` while unrelated stale snapshots are pruned.

Expected smoke diagnostics are bounded `M007_CLOSEOUT_START`, `M007_CLOSEOUT_PASS`, `M007_CLOSEOUT_FAIL`, `M007_CLOSEOUT_RESULT`, `M007_OUTBOX_FAILURE_OK`, `M007_OUTBOX_HEALTH_OK`, `M007_OUTBOX_RECOVERY_OK`, `M007_OUTBOX_DEAD_LETTER_OK`, `M007_OUTBOX_QUARANTINE_OK`, and `M007_OUTBOX_RETENTION_OK` marker lines. The success details include `counts_by_state=retrying:1`, `retry_reason=matrix_upload_timeout`, `phases=text:delivered,upload:pending,image:pending`, `state=delivered`, `skipped=text called=upload,image`, `state=dead_lettered`, `dead_letter_reason=matrix_upload_http_403`, `later_attempted=0`, `reason=invalid_json`, `quarantined_count=1`, `retained_original_upload=true`, and `stale_pruned=true`.

This smoke uses fake Matrix clients inside the container snippets and does not exercise live RTSP capture, live Matrix sync/delivery, live media upload, detector/model inference, cloud services, real credentials, or real operator runtime artifacts. Redaction checks must keep placeholder camera/token values, Authorization headers, raw Matrix response bodies, tracebacks, raw image bytes, corrupt payload bytes, and unbounded Docker output out of published diagnostics. Operators should inspect health JSON first, cockpit summaries second, closeout markers and bounded runner output for proof, and raw outbox/quarantine artifacts only as a local fallback.
...
Authorized Matrix command senders configured in `matrix.command_authorized_senders` can inspect the local monitor cockpit and correct the local vehicle-history archive from the parking room. Use `!parking help` to list available commands. Empty `matrix.command_authorized_senders` default-denies all Matrix commands, including read-only cockpit commands, so the parking room is not an unauthenticated runtime surface.

The read-only cockpit commands are `!parking status`, `!parking config`, `!parking latest`, `!parking why <spot_id>`, `!parking explain <spot_id>`, `!parking recent`, `!parking confidence`, and `!parking at <time> <spot_id>`. `!parking status` reads bounded local `health.json` and `state.json` summaries and reports current health, runtime-loop freshness, spot states, failure counters, quiet-window markers, and safe error classes such as `error_type`. Missing, corrupt, or unreadable health/state files are reported as `unavailable`; old health timestamps are called `stale`; missing timestamps are reported with unknown freshness rather than pretending the monitor is healthy. `!parking config` summarizes the loaded safe operator configuration: detector threshold names and values, frame size, runtime interval, retention count, data/state/health/snapshot paths, quiet windows, spot names and polygon point counts, command prefix, authorized-sender count, and whether the Matrix token is configured.

`!parking latest` sends a concise runtime summary plus one Matrix image only when the already-existing local `latest.jpg` passes validation. The summary starts with `Parking monitor latest` when the image is available, includes `Snapshot: fresh raw latest.jpg` with JPEG dimensions, byte size, and modified age, then reports the same bounded health/state signals used by the cockpit: health freshness including `stale` when runtime health is old, iteration, last-frame age, decode mode, capture/detection failure counts, and per-spot decisions. If `latest.jpg` is missing, unreadable, too large, not named exactly `latest.jpg`, or cannot be parsed as a JPEG, the reply is text-only and starts with `Parking monitor latest unavailable` plus a safe `Snapshot: unavailable` diagnostic such as `missing`, `too large`, or `invalid JPEG`.

`!parking why <spot_id>` and `!parking explain <spot_id>` explain the bounded recent decision memory for one configured spot, such as accepted/rejected evidence, hit/miss streak context, quiet-window or weak-open suppression, alert outcomes, and safe command/lab outcomes when those records exist. Invalid spot IDs or extra arguments are rejected with usage/validation wording before any memory read. Missing, corrupt, oversized, unsupported, or unreadable `operator-decision-memory.json` is reported as `Decision memory unavailable`; a corrupt or oversized memory artifact may be quarantined locally with an `operator-decision-memory-quarantined` diagnostic, and the reply states that no detector or camera work was run. Unknown spots or spots without retained records return `No recent decision memory for this spot` rather than scraping logs or re-running detection.

`!parking recent` returns a compact bounded timeline from the same local `operator-decision-memory.json` artifact. It may include decision, alert, suppression, miss, command outcome, and detection-lab outcome records, clipped to the implementation reply and retention limits. Empty memory returns `No recent decision memory`; missing/corrupt/unreadable memory returns `Decision memory unavailable`; unavailable current health/state remains `unavailable` or `stale` in commands that include those surfaces rather than being inferred from decision memory. The timeline is explanatory evidence only, not an audit log or complete video history.

`!parking confidence` shows artifact-derived spot stability, weak evidence, timeline health, and Matrix delivery status. It reads only local retained artifacts: loaded config, `health.json`, `state.json`, bounded decision memory, and `data/timeline/frames/` filenames. It reports stable/weak/unavailable spot status from copied occupancy-state summaries, recent weak evidence such as confidence dips or suppressions, whether timestamped timeline frames exist, and recent Matrix delivery memory. Missing or malformed health/state/memory/timeline artifacts degrade to `unavailable`, `decision memory unavailable`, or `no timestamped retained frames found` wording; the command remains text-only and does not run detector/model inference, camera capture, media upload, alert emission, threshold mutation, cloud work, or live state mutation.

`!parking analytics` is a read-only historical occupancy summary over local vehicle-history artifacts. Use `!parking analytics` or `!parking analytics 7d` for the default seven-day window, `!parking analytics today` for the current local day, `!parking analytics 30d` for the last thirty days, and `!parking analytics all` for all retained sessions. The reply is text-only and may include per-spot session counts, occupied duration, average dwell, longest session, current occupancy context, and sparse-data or active-session caveats from local `data/vehicle-history/` records plus current state when available. Invalid windows are rejected with `usage: !parking analytics [today|7d|30d|all]` before reading analytics artifacts. Analytics does not start capture, run detector/model inference, upload media, emit alerts, mutate feedback labels, mutate vehicle-history records, change thresholds, prove a live camera, or prove live Matrix room delivery.

Use `!parking at <time> <spot_id>` to review an incident around a retained timeline frame. The exact grammar is time first, spot second: for example, `!parking at 7:00pm left_spot` or `!parking at 2026-05-18T19:00:00Z left_spot`. The command searches the local `data/timeline/frames/` buffer for the nearest timestamped frame, may run local detector replay only for this incident-review path when a retained frame and replay components are available, uses a copied occupancy-state simulation rather than live state, and includes bounded recent decision-memory context. Missing timeline frames return `Nearest retained frame: unavailable` and `No retained timeline frames were found`; corrupt or unreadable frames and malformed state/memory artifacts are reported with bounded unavailable/degraded wording instead of tracebacks. The command may attach a resized Matrix image derived from the retained frame when validation succeeds; it never captures a new camera frame, mutates live occupancy state, vehicle-history records, alert history, detector thresholds, Matrix open-spot alert delivery, cloud services, snapshot retention, or config files.

Use `!parking correct <spot_id> <open|occupied>` or its explicit false-alert alias `!parking false-alert <spot_id> <open|occupied>` when the latest Matrix report for a spot is wrong. For example, if the monitor says `Parking spot occupied: left_spot` but the alert image shows the left spot is empty, send `!parking false-alert left_spot open` or `!parking correct left_spot open`. The command is available only to authorized Matrix command senders, records a bounded local label in `data/operator-feedback-labels.json`, links that label to the most recent retained alert snapshot for the spot when available, and adds a compact feedback outcome to decision memory. The label stores metadata such as reported state, actual state, correction time, alert event type, and safe JPEG metadata; it does not store image bytes, camera URLs, Matrix tokens, raw Matrix bodies, or tracebacks.

Use `!parking learn <spot_id> <open|occupied> at <time>` or its explicit missed-alert alias `!parking missed-alert <spot_id> <open|occupied> at <time>` to record a calibration-review label against the nearest retained timeline frame instead of a Matrix alert. For example, `!parking missed-alert left_spot occupied at 2026-05-18T19:00:00Z` validates the retained local JPEG metadata, replays bounded local context for that time when available, and appends one idempotent learn label plus a compact feedback memory record. Duplicate Matrix event replays repeat the bounded acknowledgement without creating a second label. Missing/corrupt timeline evidence, invalid spot IDs, invalid states, and invalid times are reported with safe not-recorded wording and do not mutate alert history, live occupancy state, detector thresholds, Matrix open-spot alerts, or cloud services.

Feedback labels are training and replay evidence only. `!parking correct` and `!parking false-alert` do not mutate live occupancy state, vehicle-history identity state, quiet-window markers, detector thresholds, polygons, model config, or alert delivery; the feedback path does not mutate live occupancy state. `!parking learn` and `!parking missed-alert` follow the same no-live-mutation boundary for retained-timeline calibration labels. They append local feedback labels only after validating the bounded alert or timeline evidence they can access. It does not automatically change detector thresholds or train a model, and reviewed labels must flow through the detection-lab replay/tuning evidence gate before any production tuning change.
...
The Matrix detection-lab commands are authorized cockpit commands for bounded local replay/tuning only: `!parking lab run replay`, `!parking lab run tuning`, `!parking lab status`, and `!parking lab status <job_id|latest>`. They use the same authorization/default-deny boundary as the other Matrix commands, so an empty `matrix.command_authorized_senders` list denies lab starts and lab status lookups before any lab artifact is touched. `!parking help` lists the exact lab grammar; extra arguments, unsupported lab kinds, malformed job IDs, path traversal strings, URLs, shell snippets, and Matrix-supplied filesystem paths are rejected or ignored by the parser/status lookup boundary.

`!parking lab run replay` starts an asynchronous, non-blocking local replay job and returns immediately with a bounded job ID such as `lab-YYYYMMDDTHHMMSSZ-xxxxxxxx`. `!parking lab run tuning` does the same for a local tuning comparison job. Both commands read only fixed local detection-lab inputs under the effective runtime data directory: replay uses `data/detection-lab/labels.json` and `data/detection-lab/replay-config.json`; tuning uses `data/detection-lab/labels.json`, `data/detection-lab/baseline-config.json`, and `data/detection-lab/proposed-config.json`. Matrix text never chooses labels, configs, output directories, model names, camera URLs, command arguments, shell commands, scripts, or arbitrary report paths.

Lab outputs are contained under `data/detection-lab/jobs/<job_id>/` with a persisted redacted `status.json` and, on success, a local `replay-report.json` or `tuning-report.json` summary in that job directory. `!parking lab status` is the same as `!parking lab status latest`; `!parking lab status <job_id|latest>` summarizes the latest or selected persisted status/report artifact with safe fields such as job id, kind, status, phase, timestamps, relative report name, status counts, coverage counts, tuning decision, metric deltas, missing fixed inputs, and redaction-scan counts. Missing lab jobs, missing fixed inputs, corrupt or unreadable status files, malformed reports, or unavailable runners are reported with safe unavailable/blocked wording such as `Detection lab status unavailable`, `missing_fixed_inputs`, `status_unreadable`, `malformed_report`, or `runner_unavailable` rather than tracebacks or raw file contents.

Detection-lab Matrix replies are text-only and do not upload media. Starting or checking a lab job does not mutate live occupancy, health, state, quiet-window markers, snapshot retention, vehicle-history records, archive corrections, Matrix cursor state beyond the normal command poll, camera capture, live detector/model execution, or live Matrix delivery. Lab jobs are for local replay/tuning artifacts only; they do not run a live camera, change production thresholds, create per-spot runtime schema, or make a live monitoring decision from Matrix. Completed, failed, or blocked lab jobs can append safe `lab_outcome` records to `operator-decision-memory.json`, so `!parking recent` may show lab outcomes without exposing raw labels, images, tracebacks, host paths, RTSP URLs, Matrix tokens, Authorization headers, raw Matrix response bodies, or image bytes.

The latest image boundary is deliberately narrow. `!parking latest` reads the current raw full-frame `data/latest.jpg`; it must not use `data/debug_latest.jpg`, polygon overlays, calibration bundles, event snapshots, or any caller-supplied path. It does not invoke a new capture, detector/model run, upload preparation shell, live proof, or dashboard workflow; it answers from already loaded config and already persisted local runtime files. The image upload reads only the validated local JPEG bytes needed for the Matrix `m.image` response, and the Matrix image body is `Raw full-frame latest.jpg evidence`. The command does not create or prune retained files under `data/snapshots/`; those retention boundaries remain for Matrix event/live-proof snapshots, not cockpit latest reads.

The why/explain/recent memory boundary is also deliberately narrow. These commands read only the bounded local `operator-decision-memory.json` under the effective runtime data directory; Matrix arguments cannot choose arbitrary files, directories, shell commands, URLs, dashboards, snapshots, health/state paths, or detection-lab inputs. They are text-only commands: they do not upload media, mutate archive corrections, write vehicle-history records, start capture, run the detector/model, invoke detection-lab work, call Matrix sync beyond the normal command poll, or create/prune runtime artifacts.

The read-only cockpit replies are intentionally safe and bounded. They redact resolved camera URLs, Matrix access-token values, Authorization headers, raw Matrix response bodies, tracebacks, YAML dumps, image bytes, raw JPEG bytes, raw Matrix event bodies, unbounded detector payloads, and nested diagnostic values; they name environment variable keys such as `RTSP_URL` and `MATRIX_ACCESS_TOKEN` only when configuration needs to be discussed. Those read-only commands do not start capture, detector, model, upload preparation, Matrix-sync, shell, browser, dashboard, detection-lab, or live-proof work; they only answer from already loaded config and local runtime files. `!parking who` is the explicit bounded-capture exception: it may attempt one fresh raw capture, but does not run detector/model inference or mutate live occupancy state. The read-only cockpit commands never mutate live spot state, vehicle-history records, archive corrections, snapshot retention, config files, or runtime artifacts.

The mutating archive commands are separate and require the same sender authorization. Use `!parking owner <spot_id>` when the active vehicle in a spot is known to be the configured owner vehicle; for example, `!parking owner right_spot` assigns the active right-spot session to the owner profile with confidence `1.0` and adds that crop as a trusted owner-profile sample. Existing correction commands remain available: `!parking wrong <spot_id|session_id>` records a false match, and `!parking profile summary <profile_id>` returns a safe profile summary. The read-only cockpit commands never mutate live spot state or vehicle-history records; the owner/wrong/profile commands are the explicit mutation boundary.
