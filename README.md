...
The copied `config.yaml` is ignored by Git and operator-owned. Runtime code does not rewrite it. Existing operator configs may omit the adaptive resource keys and receive the validated model defaults until the operator chooses explicit overrides; `config.yaml.example` is the tracked reference for those defaults. `config.yaml` should contain environment variable names only. The service resolves these required variables at startup; provide them through your shell, service manager, or ignored `.env` file used by the live-proof wrapper:
