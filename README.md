...
- `data/operator-decision-memory.json` is the bounded local source for `!parking why <spot_id>` and `!parking recent`. Treat it as sanitized explanatory decision memory, not raw camera evidence, a complete audit log, or a place to store secrets; corrupt or oversized versions can be quarantined and reported as unavailable.
...
Use `!parking correct <spot_id> <open|occupied>` when the latest Matrix report for a spot is wrong. For example, if the monitor says `Parking spot occupied: left_spot` but the alert image shows the left spot is empty, send `!parking correct left_spot open`. The command is available only to authorized Matrix command senders, records a bounded local label in `data/operator-feedback-labels.json`, links that label to the most recent retained alert snapshot for the spot when available, and adds a compact feedback outcome to decision memory. The label stores metadata such as reported state, actual state, correction time, alert event type, and safe JPEG metadata; it does not store image bytes, camera URLs, Matrix tokens, raw Matrix bodies, or tracebacks.

Use `!parking learn <spot_id> <open|occupied> at <time>` to record a calibration-review label against the nearest retained timeline frame instead of a Matrix alert. For example, `!parking learn left_spot occupied at 2026-05-18T19:00:00Z` validates the retained local JPEG metadata, replays bounded local context for that time when available, and appends one idempotent learn label plus a compact feedback memory record. Duplicate Matrix event replays repeat the bounded acknowledgement without creating a second label. Missing/corrupt timeline evidence, invalid spot IDs, invalid states, and invalid times are reported with safe not-recorded wording and do not mutate alert history, live occupancy state, detector thresholds, Matrix open-spot alerts, or cloud services.

Feedback labels are training and replay evidence only. `!parking correct` does not mutate live occupancy state, vehicle-history identity state, quiet-window markers, detector thresholds, polygons, model config, or alert delivery. `!parking learn` follows the same no-live-mutation boundary for retained-timeline calibration labels. It does not automatically change detector thresholds or train a model; reviewed labels must flow through the detection-lab replay/tuning evidence gate before any production tuning change.
